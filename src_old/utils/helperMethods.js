...
const { generateTournamentMatch } = require("./bracketTournamentHelpers");
...
const bracketTournamentWin = async (wager, teamnum) => {
  try {
    ...
    if (round === numRounds) {
      ...
      const moneyBack = tourneyToJoin.entry_fee * tourneyToJoin.team_size;
      const restOfPrize = tourneyToJoin.prize - moneyBack;
      ...
      if (teamnum === 1) {
        ...
        for (let i = 0; i < redTeamData.usernames.length; i++) {
          ...
          redUser.balance += moneyBack / tourneyToJoin.team_size;
          await redUser.save();
        }
        ...
        for (let i = 0; i < blueTeamData.usernames.length; i++) {
          ...
          blueUser.balance += restOfPrize / tourneyToJoin.team_size;
          await blueUser.save();
        }
      } else {
        ...
        for (let i = 0; i < blueTeamData.usernames.length; i++) {
          ...
          blueUser.balance += moneyBack / tourneyToJoin.team_size;
          await blueUser.save();
        }
        ...
        for (let i = 0; i < redTeamData.usernames.length; i++) {
          ...
          redUser.balance += restOfPrize / tourneyToJoin.team_size;
          await redUser.save();
        }
      }
...
const win = async (wager, teamnum, io, callback) => {
  ...
  try {
    ...
    var earnings = parseFloat(data?.entry_fee * 0.9);
    ...
    await data.save();
    const eachUserEarnings = (prize / data.blueteam_users.length) * 0.9;
    const putUpEarnings = eachUserEarnings * winners?.length;
    ...
    if (wager?.blue_users?.includes(username)) {
      ...
      if (status == 0) {
        if (wager.redsubmit !== 0 && wager.winner == -1) {
          if (wagerdata.isTourneyMatch) {
            const newToken = await bracketTournamentWin(wager, 2);
      ...
      if (status == 0) {
        if (wager.bluesubmit !== 0 && wager.winner == -1) {
          if (wagerdata.isTourneyMatch) {
            const newToken = await bracketTournamentWin(wager, 1);
...
const resetTokenHelper = async (wagerId) => {
  ...
  const earnings = wager.entry_fee * 0.9;
  wager.done = false;
