...
const {generateRandomAvatarOptions} = require('../utils/generateRandomAvatarOptions');
const {getUser, getUserIdFromToken, getUserToken, getUsernameFromToken} = require('../utils/helperMethods');
const {getUserByName, doesUsernameMatchToken} = require('../utils/helperMethods');
        ...
        fetch('https://connect.squareup.com/v2/payments', requestOptions)
            ...
            .then(result => {
                if (result.payment['status'] === 'COMPLETED') {
                    DepositData.findOne({transactionId: result.payment['id']}, (err, deposit) => {
                        if (err) {
                            console.log(err);
                            return;
                        }
                        if (!deposit) {
                            ...
                            UserData.findOne({username: result.payment['note']}, (err, userdata) => {
                                if (err) {
                                    console.log(err);
                                }
                                if (!userdata) {
                                ...
                                console.log('Processing $' + amount / 100 + ' for ' + result.payment['note'] + "'s deposit.");
                                ...
                                userdata.save({}, (err, data) => {
                                    ...
                                    io.in(userdata.username).emit(NEW_UPDATE_BALANCE_EVENT, userdata.balance.toString());
                                });
                            });
                        }
                    });
                } else {
                    console.log('Payment failed for: ' + username + ' for ' + result?.errors[0]?.detail);
                    return res.status(409).send({error: true, message: result?.errors[0]?.detail});
                }
    ...
    fetch('https://www.poof.io/api/v1/fetch_transactions', {
        ...
        .then(data => {
            ...
            async.forEachOf(data, (value, key, callback) => {
                ...
                DepositData.find({transaction: value.transaction}, (err, depositdata) => {
                    ...
                    if (depositdata.length > 0) {
                        ...
                        UserData.findOne({username: value.name}, (err, userdata) => {
                            if (!userdata) {
                                ...
                                console.log('Processing $' + value.amount + ' for ' + userdata.username + ' deposit.');
                                var depositValue = parseFloat(value.amount);
                                userdata.balance += depositValue;
                                userdata.save({}, (err, data) => {
                                    if (err) {
                                        console.log('Could not save balance for: ' + userdata.username);
                                        return;
                                    }
                                });
...
const getUserById = async (req, res) => {
    ...
    if (userdata) {
        const doesMatchUserToken = doesUsernameMatchToken(userdata.username, userToken);
...
const getUserWithdrawals = async (req, res) => {
    ...
    const userdata = await UserData.findOne({_id: userId});
    const usernameToWithdraw = userdata?.role < 300 ? userdata?.username : req.params.username;
    ...
    try {
        ...
        if (userdata.role < 500) {
            ...
            if (withdrawalData) {
                ...
                return res.status(200).send({error: false, message: 'Successfully marked withdrawal.'});
            } else {
                return res.status(409).send({error: true, message: 'This withdrawal does not exist.'});
            }
        }
    } catch (err) {
        if (!res.headersSent) {
            return res.status(409).send({error: true, message: 'Error saving withdrawal.'});
        }
    }
};
...
const getUserDeposits = async (req, res) => {
    ...
    const userId = getUserIdFromToken(userToken);
    const userdata = await UserData.findOne({_id: userId});
    const usernameToDeposit = userdata?.role < 300 ? userdata?.username : req.params.username;
...
const setTempEpic = async (req, res) => {
    ...
    VerifyEpicData.findOne({epic: epic}, (err, epicData) => {
        if (epicData) {
            ...
            return res.status(409).send('This epic is already waiting for a verification.');
        } else {
            if (!epic || !username) {
                ...
                VerifyEpicData.findOne({username: username}, (err, epicData) => {
                    if (epicData) {
                        if (epicData.epic) {
                            ...
                            return res.status(409).send('You already have a pending Epic Verification.');
...
const resetTempEpic = (req, res) => {
    ...
    VerifyEpicData.findOne({username: username}, (err, epicData) => {
        if (err) {
            ...
            return res.status(200).send('Successfully reset your temporary Epic verification process.');
        } else {
            // console.log("no epic data found");
            return res.status(409).send('Cannot find your temporary epic verification data.');
        }
    });
};
...
const getTempEpic = (req, res) => {
    ...
    VerifyEpicData.findOne({username: username}, (err, epicData) => {
        ...
        if (epicData) {
            return res.status(200).send({error: null, tempEpic: epicData.epic, id: epicData.id});
        } else {
...
const makeWithdrawal = async (req, res, io) => {
    ...
    if ((email.includes('@') || email.includes('.')) && isPaypal === false) {
        return res.status(409).send({
            error: true,
            message: 'Your Cash Tag cannot be an email. We do not support Paypal withdrawals.',
        });
    }
    // console.log(amount_input);
    if (isNaN(amount_input)) {
        return res.status(409).send({error: true, message: 'Amount must be a number.'});
    }
    ...
    if (wager) {
        ...
        if (!data) {
            return res.status(409).send({error: true, message: 'You are not logged in.'});
        }
        ...
        if (amount > max) {
            return res.status(409).send({
                error: true,
                message: 'You have only won ' + num_formatter.format(max) + ', that is the maximum amount you can withdraw.',
            });
        }
        ...
        await data.save();
        io.in(data.username).emit(NEW_UPDATE_BALANCE_EVENT, data.balance.toString());
        return res.status(200).send({
            error: null,
            message: 'Successfully withdrew $' + newAmount + ' to ' + email + ', please expect payment within 1-7 business days.',
        });
    }
};
...
const setValUser = async (req, res) => {
    ...
    if (wager) {
        return res.status(409).send({error: true, message: 'Cannot set IDs while in a token.'});
    }
...
function verifyWebhook(payload, hmac) {
    ...
    const genHash = crypto.createHmac('sha256', process.env.SHOPIFY_SECRET).update(message).digest('base64');
    // console.log(genHash);
    return genHash === hmac;
}
