...
const getWagers = (req, res) => {
  ...
  ActiveWagerData.find({}, "wagerid", (err, activewagers) => {
    ...
    WagerData.find(
      { _id: { $in: activewagers.map((m) => m.wagerid) } },
      "blueteamid redteamid blueteam_users redteam_users entry_fee region match_type team_size first_to console_only done cancelled password game",
      (err, wagers) => {
        let filters = [];
...
const createWager = async (req, res, io) => {
  ...
  if (!req.body.first_to && game !== "CLASH") {
