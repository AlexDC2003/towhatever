const {
  NewNotificationData,
} = require("../models/NewNotifications");
...
const clearNotificationHelper = async (username, callback) => {
  if (!username) {
    return res?.status(200)?.send({error: true, message:"No username involved with notification."});
  }
...
const clearNotifications = async (req, res) => {
  ...
  const notifdata = await UserData.findOne({username: username});
    ...
    return res.status(200)?.send({error: null, message:"Successfully cleared notifications."});
};
...
const dismissNotification = async (req, res) => {
  ...
  if (!username) {
    return res?.status(409)?.send({error: true, message:"No username associated with noti."});
  }

  const userData = await UserData.findOne({username: username});


  const newNotis = userData.notifications.filter((noti) => new Date(noti?.timestamp)?.toString() !==
