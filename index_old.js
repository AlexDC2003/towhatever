...
const {login, register, renewToken, logout, verifyUserEmail} = require('./src/controllers/authenticate.js');
const {
    getWagers,
    getFirstThreeWagers,
    createWager,
...
const {createBracketTournament, joinBracketTournament, startBracketTournament, cancelBracketTournament} = require('./src/controllers/bracketTournaments');
const {
    resolve,
    win,
    submit,
...
const {createTeam, leaveTeam, joinTeam, kickUser} = require('./src/controllers/teams');
...
const LIVE_DB = '@cluster0.nc5u5.mongodb.net/Tokens?retryWrites=true&w=majority';
const TEST_DB = '@cluster0.6eddh.mongodb.net/myFirstDatabase?retryWrites=true&w=majority';

mongoose.connect('mongodb+srv://arya:' + pwd + LIVE_DB, {
...
const {getNotifications, clearNotifications, dismissNotification} = require('./src/controllers/notifications.js');
...
function resetNotis() {
    UserData.find({}, (err, data) => {
        data.forEach(obj => {
            var x = [...new Map(obj.notifications.map(item => [item['data'], item])).values()];
            obj.notifications = [];
            obj.save({}, (err, data) => {
                obj.notifications = x;
                obj.save();
...
function checkBalance() {
    ...
    UserData.find({}, (err, data) => {
        data.forEach(obj => {
            ...
            if (data) {
                if ((obj.role == 1 || obj.role == 2) && obj.balance <= 600 && obj.balance > 0) {
            ...
            data.forEach(obj => {
                ...
                if (obj.balance >= 1) {
                    if (obj.max_withdrawal > obj.balance) {
                        ...
                        obj.save({}, (error, data) => {
                            console.log(obj.username + ' ' + obj.max_withdrawal + ' set to ' + obj.balance);
...
function sendVerificationEmail(userEmail, verifyUrl) {
    transporter
        .sendMail({
            ...
            html: emailString.getEmailString().replace('{#verification_code#}', verifyUrl).replace('{#discord_invite#}', 'https://www.discord.gg/tknsgg'), // html body
        })
        .then(info => {
            // console.log({ info });
        })
...
function sendForgotPwEmail(email, verifyUrl) {
    ...
    transporter
        .sendMail({
            ...
            html: emailStringPw.getEmailStringPw().replace('{#verification_code#}', fullUrl).replace('{#discord_invite#}', 'https://www.discord.gg/tknsgg'), // html body
        })
        .then(info => {
            // console.log({ info });
        })
...
app.get('/api/wagers/:region?/:match_type?/:team_size?/:console_only?/:game?', getWagers);
...
app.post('/api/user/deposit', async (req, res) => await shopifyDeposit(req, res, io));

app.post('/api/wagers/createWager', verifyToken, async (req, res) => await createWager(req, res, io));
app.post('/api/tourneys/createBracketTournament', createBracketTournament);
app.post('/api/tourneys/joinBracketTournament', joinBracketTournament);
app.post('/api/tourneys/startBracketTournament', startBracketTournament);
app.post('/api/tourneys/cancelBracketTournament', cancelBracketTournament);
    ...
    getUserByName(username, userdata => {
        getWagerObject(wagerId, wagerdata => {
            if (userdata?.role < 200) {
                if (!(wagerdata?.blue_users?.includes(username) || wagerdata?.red_users?.includes(username))) {
            ...
            if (wagerdata?.state === wagerdata?.READY_STATE || wagerdata?.state === wagerdata?.JOIN_STATE || userdata?.role > 100) {
                ...
                cancelSocketHelper(wagerdata, newwager => {
                    if (!newwager) {
                        return res.status(409).send({error: true, message: 'Unable to cancel token!'});
                    }
                    // THE RIGHT ONE
...
app.post('/api/wager/join/:wagerId', verifyToken, async (req, res) => await joinWager(req, res, io));
app.post('/api/wager/acceptRematch', verifyToken, async (req, res) => await acceptRematch(req, res, io));
...
app.post('/api/user/punish/:userToPunish', verifyToken, async (req, res) => await punishUser(req, res, io));
...
app.post('/api/squareIsGangStyll', async (req, res) => await squareIsGangStyll(req, res, io));
...
app.post('/api/user/setAvatar', verifyToken, async (req, res) => await changeUserAvatar(req, res, io));
        ...
        fetch('https://api.fortnitetracker.com/v1/profile/all/' + id, {
            ...
            .then(fortniteApiData => {
                ...
                if (!fortniteApiData?.epicUserHandle) {
                    return res.send({
                        error: true,
                        message: 'Your Fortnite Tracker must be on public to link your Epic account.',
                    });
                }
...
app.post('/api/user/makeWithdrawal', verifyToken, async (req, res) => await makeWithdrawal(req, res, io));
                ...
                if (userdata) {
                    userdata.setPassword(confirmPassword, function (err, data) {
                        if (err) {
                            ...
                            return res.send({
                                error: true,
                                message: 'Error setting password. Please request a new verification code.',
                            });
                        } else {
        ...
        if (data) {
            VerifyData.findOne({username: data.username}, (err, verifydata) => {
                if (verifydata) {
                    ...
                    return res.send({
                        error: false,
                        message: 'Successfully sent email. Please check your spam folder if you do not see it.',
                    });
                }
            });
        } else if (!data) {
    ...
    passport.authenticate('local', (err, user, info) => {
        ...
        getUserByName(user.username, userdata => {
            if (!userdata['role'] || userdata.role == 3 || userdata.role == 1) {
                return res.redirect('/login?info=' + 'Only Premium and Beta users can log in.');
            }
...
app.post('/register', (req, res, next) => {
    ...
    if (!/^[a-zA-Z0-9_]*$/.test(username)) {
        return res.redirect('/register?info=' + 'Username may only contain letters, numbers, and underscores.');
    }
    ...
    UserDetails.countDocuments({email: email}, (err, count) => {
        ...
        UserDetails.register({username: username, active: false}, password, function (err, user) {
            if (err) {
                if (err.toString().includes('UserExistsError')) {
                    return res.redirect('/register?info=' + 'A user with the given username is already registered.');
                }
                return next(err);
            }
            ...
            if (codeQuery) {
                ReferralData.findOne({code: codeQuery}, (err, data) => {
                    if (data) {
                        ...
                        return res.redirect('/register?info=' + 'Referral code does not exist');
                    }
                });
            } else {
                res.redirect('/login');
...
app.get('/isInWager', (req, res, next) => {
    ...
    getUser(req, res, (req, res, userdata) => {
        TeamData.findOne({usernames: userdata.username, in_wager: true}, (err, data) => {
            if (data) {
...
app.post('/addTeammate', (req, res, next) => {
    ...
    getUser(req, res, (req, res, userdata) => {
        TeamData.findOne({_id: teamid}, (err, data) => {
            ...
            sendNotification(teammate, userdata.username + ' sent you an invite.', true, ACTION_JOIN_TEAM, teamid, res);
        });
    });
});
            ...
            if (noti.action == ACTION_JOIN_TEAM) {
                ...
                getTeam(targetTeam, destTeam => {
                    if (destTeam.in_wager) {
                        res.status(409).send('You cannot join this team while they are in a match.');
                        return;
                    }

                    TeamData.countDocuments({usernames: userdata.username}, (err, count) => {
                        if (count >= 10) {
                            return res.status(409).send('You may only be in 10 teams at one time! Please leave a team.');
                        }
...
app.post('/createWager', (req, res, next) => {
    ...
    if (!(region == 'NAE' || region == 'NAW' || region == 'EU' || region == 'OCE')) {
        res.status(409).send('Region must be NAE, NAW, EU, or OCE.');
        return;
    }
    if (!(match_type == 'ZW' || match_type == 'REAL' || match_type == 'BOX')) {
        res.status(409).send('Match type must be Zone Wars, Realistics, or Boxfights.');
        return;
    }
        ...
        getTeam(team, teamdata => {
            ...
            teamMinBal(usersToCheck, -1, minBalance => {
                if (entry_fee > minBalance) {
                    return res.status(409).send("At least one of the team's members does not have a high enough balance.");
                }

                teamCheckEpics(JSON.parse(JSON.stringify(teamdata.usernames)), epicsSet => {
                    if (epicsSet == -1) {
                        return res.status(409).send("At least one of the team's members must submit their epic username on their profile.");
                    } else if (epicsSet == -2) {
                        return res.status(409).send("At least one of your team's members is banned.");
                    }
...
function tick(wagerObj) {
    ...
    wagerObj.timer -= 1;
    if (wagerObj.state == wagerObj.READY_STATE && wagerObj.timer === 0) {
        // //// console.log("cancelled ongoing wager because non ready up ")
        // cancel(wagerObj);
        return;
    } else if (wagerObj.state == wagerObj.PLAYING_STATE && wagerObj.timer === 0) {
        ...
        //   if (wagerObj.bluesubmit < 0 && wagerObj.redsubmit < 0) {
        //     return;
        //   }
        //   // console.log("PLAYING STATE AND TIMER 0");
        //   // give the auto-win
        //   if (wagerObj.bluesubmit >= 0 && wagerObj.bluesubmit != -1) {
        //     // console.log("auto win blue");
        //     resolve(wagerObj, 1, 0); // blue win
        //   } else if (wagerObj.redsubmit >= 0 && wagerObj.redsubmit != -1) {
        //     // console.log("auto win red");
        //     resolve(wagerObj, 0, 1); // red win
        //   } else {
        //     reset(wagerObj.wagerid);
        //   }
        //   // console.log("wager marked done but in tick function");
        //   // mark wager as complete
        //   markWagerDone(wagerObj.wagerid);

        //// console.log("BLUE SUBMIT HERE BEFORE SAVING WAGER" + wagerObj.bluesubmit);
    }
...
function tickWagers() {
    ...
    WagerObjectData.find({winner: -1, timer: {$gt: 0}}, (err, all_wager_objs) => {
...
app.get('/wager/*', (req, res) => {
    ...
    WagerData.findOne({_id: req.originalUrl.split('wager/')[1]}, (err, wagerdata) => {
        if (!wagerdata) {
            return res.redirect('/dashboard');
        }
        ...
        getUser(req, res, (req, res, userdata) => {
            if (role < 100) {
                if (!(wagerdata.blueteam_users.includes(userdata.username) || wagerdata.redteam_users.includes(userdata.username))) {
                    return res.redirect('/dashboard');
                }
            }
            ...
            res.render('wager.ejs', {
                ...
                account_name: userdata.username,
                balance: currencyFormatter.format(userdata.balance),
                team_size: wagerdata.team_size.toString() + 'v' + wagerdata.team_size.toString(),
                match_type: wagerdata.match_type,
...
app.post('/wagerStatus/*', (req, res) => {
    ...
    getWagerObject(wager_id, data => {
        if (data.state == data.CANCEL_STATE) {
            return res.send('{"state": ' + data.CANCEL_STATE.toString() + ', "CANCEL_STATE": ' + data.CANCEL_STATE.toString() + ' }');
        }
...
app.get('/verify', (req, res) => {
    ...
    VerifyData.findOne({code: currentCode}, (err, data) => {
        if (data) {
            ...
            return res.redirect('/register?info=' + 'Error verifying account. Please contact an admin.');
        }
    });
});

app.get('/verify', (req, res) => res.sendFile('/public/verify.html', {root: __dirname}));

app.get('/resetPassword', (req, res) => res.sendFile('/public/reset_password.html', {root: __dirname}));
...
app.post('/punish', (req, res) => {
    ...
    getUser(req, res, (req, res, userdata) => {
        if (!userdata.role) {
            return res.status(409).send('You do not have permission for this action.');
        }
...
app.post('/withdraw', (req, res) => {
    ...
    getUser(req, res, (req, res, data) => {
        ...
        if (amount > max) {
            return res.status(409).send('You have only won ' + num_formatter.format(max) + ', that is the maximum amount you can withdraw.');
        }
        if (amount > data.balance) {
            return res.status(409).send('You do not have a high enough balance for this.');
        }

        if (amount < 10) {
            return res.status(409).send('You may only withdraw amounts greater than $10.');
        }
